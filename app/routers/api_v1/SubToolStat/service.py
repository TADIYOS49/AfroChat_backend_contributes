...
async def get_sub_tools_service(
    ...
    count_query = text(
        f"""
        SELECT
            COUNT(DISTINCT sub_tool.sub_tool_id) AS total_count
        FROM
            sub_tool
        JOIN
            (
    ...
    response = Paginate[SubToolStat](meta_data=Meta(**meta_data), data=items)

    return response


async def search_sub_tools_service(
    search_query: str,
    start_date: date,
    end_date: date,
    limit: int,
    ...
    sql_query = text(
        ...
        JOIN
            ...
            ON sub_tool.sub_tool_id = chat_session.sub_tool_id
        WHERE
            LOWER(sub_tool.sub_tool_name) LIKE '%{search_query.lower()}%'
        GROUP BY
            sub_tool.sub_tool_id, sub_tool.sub_tool_name
        ORDER BY
            {order_by.value}
    ...
    count_query = text(
        ...
        JOIN
            ...
            ON sub_tool.sub_tool_id = chat_session.sub_tool_id
            AND LOWER(sub_tool.sub_tool_name) LIKE '%{search_query.lower()}%'
        """
    )
...
async def sub_tool_stat_total_service(
    sub_tool_stat_request: SubToolStatRequest,
    db_session: AsyncSession,
    include_developers: bool = False,
    include_archived: bool = False,
):
    """function to return the sub tool daily stats"""
    ...
    def build_query(
        ...
        sub_tool_id: str = "",
        sub_tool_id_list_to_str: str = "",
    ):
        """function to build the sql query"""
        sub_tool_filter = (
            f"AND sub_tool.sub_tool_id = '{sub_tool_id}'"
            if sub_tool_id
            else f"AND sub_tool.sub_tool_id IN ({sub_tool_id_list_to_str})"
        )
    ...
    for sub_tool_id in sub_tool_stat_request.sub_tool_ids:
        sub_tool = (
            await db_session.execute(
                select(SubTool).filter(SubTool.sub_tool_id == sub_tool_id)
            )
        ).scalar()
        ...
        sql_query = build_query(
            start_date=sub_tool_stat_request.start_date,
            end_date=sub_tool_stat_request.end_date,
            price_per_token=0.0001,
            sub_tool_id=str(sub_tool_id),
        )
        ...
        sub_tool_stats.append(
            SubToolStatResponse(
                sub_tool_id=sub_tool_id,
                sub_tool_name=sub_tool.sub_tool_name,
                daily_stats=sub_tool_stat,
            )
        )

    sub_tool_id_list_to_str = ", ".join(
        [f"'{str(sub_tool_id)}'" for sub_tool_id in sub_tool_stat_request.sub_tool_ids]
    )
    total_query = build_query(
        start_date=sub_tool_stat_request.start_date,
        end_date=sub_tool_stat_request.end_date,
        price_per_token=0.0001,
        sub_tool_id_list_to_str=sub_tool_id_list_to_str,
    )
    ...
    return SubToolStatTotalResponse(
        sub_tool_stats=sub_tool_stats, total_stat=total_stat
    )


async def get_sub_tool_users_service(
    sub_tool_stat_request: SubToolStatRequest,
    db_session: AsyncSession,
    include_developers: bool = False,
    include_archived: bool = False,
    ...
    sql_query = text(
        ...
        SELECT 
            ...
            AND sub_tool.sub_tool_id IN ({sub_tool_id_list_to_str})
            ...
            COUNT(DISTINCT CASE WHEN sub_tool.sub_tool_id IN ({sub_tool_id_list_to_str})
            THEN chat_session.user_id END) AS total_number_of_users
        FROM
            (SELECT generate_series(
                    '{sub_tool_stat_request.start_date}',
                    '{sub_tool_stat_request.end_date}',
                    '1 day'::interval
                )::date AS date
            ) AS d
        LEFT JOIN
...
async def get_chat_session_counts_service(
    ...
    sql_query = text(
        ...
        SELECT
            ...
            COALESCE(COUNT(DISTINCT CASE WHEN sub_tool.sub_tool_id IN ({sub_tool_id_list_to_str})
...
async def export_to_excel_service(
    ...
    SELECT
        sub_tool.sub_tool_id AS sub_tool_id,
        sub_tool.sub_tool_name AS sub_tool_name,
    ...
    LEFT JOIN
        ...
        ON sub_tool.sub_tool_id = chat_session.sub_tool_id
    GROUP BY
        sub_tool.sub_tool_id, sub_tool.sub_tool_name, date_series.date
    ORDER BY
        sub_tool.sub_tool_id, date_series.date;
    """
    )
    ...
    # Pivot the DataFrame
    pivot_df = df.pivot(
        index="sub_tool_name", columns="date", values="session_count"
    ).fillna(0)
    ...
    # Pivot the DataFrame
    pivot_df = df.pivot(
        index="sub_tool_name", columns="date", values="message_count"
    ).fillna(0)
    pivot_df.columns = pd.to_datetime(pivot_df.columns).strftime("%Y-%m-%d")
    ...
    # Pivot the DataFrame
    pivot_df = df.pivot(
        index="sub_tool_name", columns="date", values="number_of_users"
    ).fillna(0)
    pivot_df.columns = pd.to_datetime(pivot_df.columns).strftime("%Y-%m-%d")
    ...
    # Pivot the DataFrame
    pivot_df = df.pivot(
        index="sub_tool_name", columns="date", values="token_usage"
    ).fillna(0)
    pivot_df.columns = pd.to_datetime(pivot_df.columns).strftime("%Y-%m-%d")
